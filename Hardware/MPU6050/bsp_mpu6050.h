...
#define SDA_OUT()   {                                                  \
                        DL_GPIO_initDigitalOutput(MPU_MPU_SDA_IOMUX);     \
                        DL_GPIO_setPins(MPU_PORT, MPU_MPU_SDA_PIN );      \
                        DL_GPIO_enableOutput(MPU_PORT, MPU_MPU_SDA_PIN ); \
                    }
//设置SDA输入模式 Set SDA input mode
#define SDA_IN()    { DL_GPIO_initDigitalInput(MPU_MPU_SDA_IOMUX); }

//获取SDA引脚的电平变化 Get the level change of the SDA pin
#define SDA_GET()   ( ( ( DL_GPIO_readPins(MPU_PORT,MPU_MPU_SDA_PIN ) & MPU_MPU_SDA_PIN  ) > 0 ) ? 1 : 0 )
//SDA与SCL输出 SDA and SCL output
#define SDA(x)      ( (x) ? (DL_GPIO_setPins(MPU_PORT,MPU_MPU_SDA_PIN )) : (DL_GPIO_clearPins(MPU_PORT,MPU_MPU_SDA_PIN )) )
#define SCL(x)      ( (x) ? (DL_GPIO_setPins(MPU_PORT,MPU_MPU_SCL_PIN )) : (DL_GPIO_clearPins(MPU_PORT,MPU_MPU_SCL_PIN )) )
