...
{
    uint32_t cnt, dist;
    DL_GPIO_setPins(USOUND_PORT, USOUND_USOUND_RX_PIN);
    delay_cycles(CPUCLK_FREQ/50000);
    DL_GPIO_clearPins(USOUND_PORT, USOUND_USOUND_RX_PIN);

    while(!DL_GPIO_readPins(USOUND_PORT, USOUND_USOUND_TX_PIN));

    overflowFlag = 0;//溢出标志位
    DL_TimerG_setTimerCount(TIMER_USOUND_INST, 0);//计数置0
    DL_TimerG_startCounter(TIMER_USOUND_INST);//开始计数

    while(DL_GPIO_readPins(USOUND_PORT, USOUND_USOUND_TX_PIN) && !overflowFlag);//溢出或受到回波跳出
